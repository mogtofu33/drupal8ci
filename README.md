# Drupal CI Docker image for Gitlab CI

## Details

[Drupal](https://www.drupal.org) ci image based on official [docker Drupal](https://github.com/docker-library/drupal)
...
To use with a local Drupal managed by Composer, mount your Drupal on `/opt/drupal/`
...
CI variable `DO_RELEASE`, default to `1` to push to Docker hub.
...
```bash
docker run -it --rm mogtofu33/drupal8ci:11.x-dev-11.3 /tests/prepare-tests.sh && pytest
```
